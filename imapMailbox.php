...
// // 		$params['charset'] is always equal with 'utf-8',so there will be a warning;
...
// 		}
		...
		if($attachmentId) {
			...
			if($this->attachmentsDir) {
				...
				$attachTypeArray = explode('-', $mail->subject);
				$attachType = $attachTypeArray['0'];
				$fileSysName = preg_replace('~[\\\\/]~', '', $attachType. '_' . $attachmentId . '_' . preg_replace(array_keys($replace), $replace, $fileName));
